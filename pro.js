(function(){
  ...
  function addStyles(){
    ...
    const s=document.createElement('style');s.id='cvgoProStyles';s.textContent=`
      ...
      #cvgoProCheckout:disabled{opacity:.55;cursor:not-allowed}
  ...
  function openPro(){
    ...
    const title=trial?'Tu prueba gratuita está activa':'Activa CVGO PRO';
    const subtitle=trial?'Tienes acceso completo durante 7 días. Después podrás continuar con PRO por 9,99 €/mes.':'Tu prueba gratuita ha terminado. Continúa usando CVGO con todas las funciones PRO.';
    const d=document.createElement('div');d.id='cvgoProModal';d.innerHTML=`<div class="box">
      ...
      <button id="cvgoProCheckout" ${trial?'disabled':''}>${trial?'Disponible al finalizar la prueba':'Activar PRO por 9,99 €/mes'}</button>
  ...
  function addControls(){
    ...
    if(header&&!$('cvgoProBtn')){const b=document.createElement('button');b.id='cvgoProBtn';b.textContent=me.trial_active?'🎁 Prueba activa':'⭐ Activar PRO';b.onclick=openPro;header.appendChild(b)}
