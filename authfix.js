(function(){
  ...
  function addMarketing(){
    ...
    panel.innerHTML=`<div class="cvgo-eyebrow">CV + IA + ATS + ENTREVISTA</div><h2>Crea un CV profesional que te ayude a conseguir <span>entrevistas.</span></h2><p class="cvgo-lead">Diseña tu CV, adáptalo a cada oferta y prepárate para la entrevista con herramientas inteligentes en un solo lugar.</p><div class="cvgo-features"><div class="cvgo-feature"><div class="cvgo-icon">🎯</div><div><strong>Adaptado a cada oferta</strong><small>Analiza los requisitos y descubre qué encaja y qué falta en tu CV.</small></div></div><div class="cvgo-feature"><div class="cvgo-icon">🤖</div><div><strong>IA para mejorar tu candidatura</strong><small>Optimiza perfil, experiencia y palabras clave sin inventar información.</small></div></div><div class="cvgo-feature"><div class="cvgo-icon">🎤</div><div><strong>Llega preparado a la entrevista</strong><small>Practica preguntas y respuestas enfocadas al puesto que buscas.</small></div></div></div><div class="cvgo-trial"><div class="cvgo-trial-icon">🎁</div><div><strong>7 días gratis · Sin compromiso</strong><span>Acceso completo. Después, desde 9,99 €/mes.</span></div></div><button type="button" class="cvgo-hero-cta" id="cvgoHeroCta">Empezar gratis →</button><span class="cvgo-price-note">No se cobra durante la prueba</span>`;
    auth.prepend(panel);
  ...
  function addBrand(){const card=document.querySelector('#auth .card');if(card&&!card.querySelector('.auth-brand')){const brand=document.createElement('div');brand.className='auth-brand';brand.innerHTML='<div class="auth-mark">CVProfit</div>';card.prepend(brand)}}
  function addConsent(){
  ...
  function applyAuthLabels(){
    ...
    if(authMode==='login'){title.textContent='Bienvenido de nuevo';text.textContent='Inicia sesión en tu cuenta de CVProfit y continúa trabajando en tus CV.';btn.textContent='Iniciar sesión →';switchText.textContent='¿Aún no tienes cuenta?';switchBtn.textContent='Crear cuenta gratis →';if(trial)trial.style.display='none';if(wrap)wrap.style.display='none';if(err)err.style.display='none'}else{title.textContent='Crea tu cuenta';text.textContent='Prueba todas las funciones de CVProfit durante 7 días.';btn.textContent='Crear mi cuenta →';switchText.textContent='¿Ya tienes cuenta?';switchBtn.textContent='Iniciar sesión';if(trial)trial.style.display='block';if(wrap)wrap.style.display='flex'}
  }
