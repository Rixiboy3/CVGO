...
def register_cover(app):
    ...
    def cover_letter_api():
        ...
        d=request.get_json(silent=True) or {}; offer=str(d.get('offer') or '').strip(); profile=d.get('profile') or {}
        if len(offer)<20:return jsonify(ok=False,error='OFFER_REQUIRED'),400
        approaches=['Empieza con un detalle concreto del puesto que tenga relación directa con mi experiencia.','Empieza desde mi situación profesional actual y cuenta de forma sencilla por qué estoy valorando este cambio.','Empieza directamente con una razón concreta por la que el puesto me puede encajar.','Empieza por una experiencia o responsabilidad del CV que sea especialmente útil para esta oferta.']
        voice_profiles=['Tono directo y profesional. Como alguien que escribe normalmente correos de trabajo.','Tono cercano y natural. Profesional, pero sin sonar demasiado formal.','Tono sencillo y seguro. Frases claras, sin intentar impresionar con vocabulario.','Tono conversacional moderado. Que parezca escrita por la persona después de leer la oferta.']
        approach=random.choice(approaches); voice=random.choice(voice_profiles); summary_sample=str(profile.get('summary') or '').strip(); summary_instruction=''
        if summary_sample:summary_instruction=f'''\n\nMUESTRA DE LA FORMA DE EXPRESARSE DEL CANDIDATO:\n{summary_sample[:1200]}\n\nUtiliza esta información únicamente para captar su tono y forma de expresarse. No la copies literalmente ni inventes información a partir de ella.'''
...
- Entre 150 y 220 palabras aproximadamente.
...
{json.dumps(profile,ensure_ascii=False)}'''
        schema={'type':'object','additionalProperties':False,'properties':{'letter':{'type':'string'}},'required':['letter']};last_error=None
        for attempt in range(1,4):
            try:
                response=client.responses.create(model=model,input=prompt,text={'format':{'type':'json_schema','name':'cvprofit_cover_letter','description':'Carta de presentación personal, natural y variable en primera persona.','schema':schema,'strict':True}})
                if getattr(response,'status',None)=='incomplete':raise RuntimeError('AI_INCOMPLETE_RESPONSE')
                result=json.loads(getattr(response,'output_text','') or '{}');letter=str(result.get('letter') or '').strip()
                if not letter:raise ValueError('Carta vacía')
                lowered=letter.lower();forbidden=['el candidato','la candidata','el/la candidato','cuenta con experiencia','está preparado/a','esta preparado/a','está preparado para','esta preparado para']
...
# Mandatory email verification loads after the application routes.
import emailverify
